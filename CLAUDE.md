...
tests/
  ...
  test_orchestrator_constraints.py # Tests for orchestrator constraints: overhead, memory, idempotency, reconnection, SDK version, session limits, agent uniqueness (Task 49)
...
- Orchestrator constraints pattern (Task 49): `check_sdk_version()` reads `claude_agent_sdk.__version__` and raises `ImportError` if < 0.1.39 (REQ-OR-054); uses `_parse_version_tuple()` for tuple comparison. `reconnect_with_backoff(client, session_id, *, max_retries=3)` is async, retries `client.connect(resume=session_id)` with `2^i` delays (1s, 2s, 4s) and logs WARNING per retry; raises last `ConnectionError` after exhausting retries (REQ-OR-052). `_dispatch_phase2_with_session_limit()` uses `asyncio.Semaphore(max_concurrent_sessions)` to limit concurrency; logs WARNING when `max_concurrent_sessions < L` (REQ-OR-056). Idempotency (REQ-OR-051) guaranteed by `run_pipeline()` creating fresh local `CostTracker`, `session_agent_map`, `failure_counts`, and `finalize_flag` on each call. Agent name uniqueness (REQ-OR-057) enforced by `_build_agents_dict()` using `str(agent_type)` as keys from `build_default_agent_configs()`. Performance (REQ-OR-048) verified by timing tests: all orchestration helpers complete in <100ms
